...
import vaultJSON from '../utils/smart-contract/abis/NooriVault.json';
import nooriTokenJSON from '../utils/smart-contract/abis/NooriToken.json';
import UserAccountContext from "./UserAccountContext";
import web3Service from '../utils/smart-contract/web3_bu';
...
const UserAccountProvider = ({ children }) => {
    ...
    const fetchSystemParams = useCallback(async () => {
        ...
        try {
            ...
            setSystemParams({
                depositRate: web3Service.web3.utils.fromWei(depositRate, 'ether'),
                borrowRate: web3Service.web3.utils.fromWei(borrowRate, 'ether'),
                transferFee: web3Service.web3.utils.fromWei(transferFee, 'ether'),
                withdrawalFee: web3Service.web3.utils.fromWei(withdrawalFee, 'ether'),
                minCollateralRatio: web3Service.web3.utils.fromWei(minCollateralRatio, 'ether')
            });
        } catch (err) {
    ...
    const initializeContracts = useCallback(async () => {
        ...
        try {
            ...
            tokenContractRef.current = web3Service.createContract(
                NooriTokenABI,
                tokenContractAddress
            );

            vaultContractRef.current = web3Service.createContract(
                NooriVaultABI,
    ...
    }, [account, tokenContractAddress, vaultContractAddress, fetchBalances, fetchSystemParams]);
