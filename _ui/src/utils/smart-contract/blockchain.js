...
const initializeContracts = async () => {
    ...
    if (!tokenContract) {
        tokenContract = web3Service.createContract(nooriTokenJSON.abi, nooriTokenCA);
    }
    ...
    return { vaultContract, tokenContract, bridgeContract };
};

// Helper to ensure contracts are initialized
const ensureContracts = async () => {
    if (!vaultContract || !tokenContract || !bridgeContract) {
...
export const tokenMintAdmin = async (to, amount) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.mint(to, amount).estimateGas({
            from: account
        });

        await tokenContract.methods.mint(to, amount).send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenPauseAdmin = async () => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.pause().estimateGas({
            from: account
        });
        
        await tokenContract.methods.pause().send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenUnpauseAdmin = async () => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.unpause().estimateGas({
            from: account
        });
        
        await tokenContract.methods.unpause().send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenFreeAccountAdmin = async (userAccount) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.freezeAccount(userAccount).estimateGas({
            from: account,
        });

        await tokenContract.methods.freezeAccount(userAccount).send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenUnfreeAccountAdmin = async (userAccount) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.unfreezeAccount(userAccount).estimateGas({
            from: account,
        });

        await tokenContract.methods.unfreezeAccount(userAccount).send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenSetMinterAdmin = async (minter, status) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.setMinter(minter, status).estimateGas({
            from: account
        });

        await tokenContract.methods.setMinter(minter, status).send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenSetPauserAdmin = async (pauser, status) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.setPauser(pauser, status).estimateGas({
            from: account
        });

        await tokenContract.methods.setPauser(pauser, status).send({
            from: account,
            gas: gasEstimate
        });
...
export const tokenSetBridge = async (bridge, status) => {
    try {
        ...
        const gasEstimate = await tokenContract.methods.setBridge(bridge, status).estimateGas({
            from: account
        });

        await tokenContract.methods.setBridge(bridge, status).send({
            from: account,
            gas: gasEstimate
        });
...
//     const gasEstimate = await tokenContract.methods.transfer(to, amount).estimateGas({from: account});
    
//     const response = await tokenContract.methods.transfer(to, amount).send({
//         from: account,
//         gas: gasEstimate
//     });
